...
- **Two-schema split (vision).** The host call passes the **relaxed** `BUG_REPORT_INPUT_SCHEMA` (no enums / no `required` / no per-field `type` / `additionalProperties: true`) so the host's pre-validation never rejects fixable output; then `vision._normalize()` **projects the report down to the known fields (dropping extra model keys)**, coerces types (severity `"high"` → `"critical"`, non-string scalars), and strips text, and `vision._validate()` re-checks against the **strict** `BUG_REPORT_SCHEMA` (`additionalProperties:false`, required + `minLength:1` on title/summary/actual_behavior) locally. The two schemas only converge because `_normalize` drops unknown keys — tightening the input schema, or skipping the projection, re-breaks normalization (a stray model key would 'vision_invalid' an otherwise-good report).
- **Secrets come only from the environment, never from config.** `config.py` env-expansion **refuses** the credential denylist (`JIRA_API_TOKEN`, `LINEAR_API_KEY`, `GITHUB_TOKEN`, **`JIRA_EMAIL`** — the basic-auth username) so a stray `${TOKEN}` can't leak into a ticket. Tokens are read in `clients.py` only when a tracker is used, and never logged or echoed in errors. Echoed tracker error bodies are control-char-scrubbed.
- **Network rules (clients.py):** HTTPS-only (non-`https://` is refused), explicit timeout on every request, bounded retries (2) on timeouts/connection errors / a fixed set of 5xx (`500/502/503/504`) / rate limits (429, or 403 with a rate-limit signal — honoring a capped `Retry-After`) **— never on a non-rate-limit 4xx**. A TLS/cert failure is a distinct `tls_error` (not retried); every other `requests.RequestException` is still mapped to a structured error (never escapes as `internal_error`). The resolved Jira **project key** (`^[A-Za-z][A-Za-z0-9_]*$`) and GitHub **`owner/name`** are validated before use (JQL-injection / URL-path safety).
- **`severity_map` is required per target; never guess a priority/label.** An unmapped severity is a `BugTicketError`. `{placeholder}` templates expand only from a fixed key set; an unknown placeholder is an error. A `severity_map`/`custom_fields` entry may **not** override a builder-owned core field (`reserved_field_override`) — no silent clobber.
